...
class trainer():
    def __init__(self, station_history, forecasting, remove_only, num_stations, num_sessions, action_space, fixed_stations, use_expected_balances_only, num_clusters, active_cluster, forecast_hours, stock_mode, IDs, assign_mode, upper_limit, lower_limit, use_expected):
        # Session Properties
        self.episodes = []
    ...
    def start(self, episodes, logging, action_space):
        ...
        for eps in self.episodes:
            ...
            for i in self.IDs:
                my_current_stock = {}
                my_current_stock[i] = self.central_server.current_stock().get(str(i))
                for j in self.IDs:
                    if j != i:
                        my_current_stock[j] = self.central_server.current_stock().get(str(j))
                #print(f"my_current_stock: {my_current_stock}")            
                self.operators[i] = agent(epsilon = 0.9, lr = 0.01, gamma = 0.9, current_stock = my_current_stock, IDs = self.IDs, action_space=action_space, expected_stock = self.central_server.get_expected_bike_stock(), forecasting = self.forecasting, remove_only = self.remove_only, forecast_hours = self.forecast_hours, id = i, use_expected = self.use_expected)#, expected_stock = self.central_server.get_expected_stock())
    ...
    def train_operator(self, idx, num_sessions, episodes):
        ...
        for eps in range(episodes): 
            ...
            while True:
                ...
                for key in self.operators.keys():
                    if self.forecasting:
                        action = self.operators.get(key).choose_action(self.central_server.get_old_stock(),self.central_server.get_expected_bike_stock(), key, current_hour)
                    else:
                        #print(f"self.central_server.get_expected_bike_stock(): {self.central_server.get_expected_bike_stock()}")
                        action = self.operators.get(key).choose_action(self.central_server.get_old_stock(),self.central_server.get_expected_bike_stock(), key, current_hour)
                    actions[key] = action
                ...
                for key in self.operators.keys():
                    if self.forecasting:
                        self.operators.get(key).learn(self.old_stocks, actions[key], my_rewards[str(key)], self.new_stocks, self.game_overs[str(key)], self.expected_stocks, key, current_hour)
                    else:
    ...
    def cal_performance(self):
        ...
        #print(f"successful_stocking: {successful_stocking}")
